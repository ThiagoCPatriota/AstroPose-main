...
class PoseDetectionGUI:
    ...
    def _build_astronaut_controls(self):
        ...
        ttk.Button(btn_frame, text="Cadastrar (iniciar sessão)", style="Accent.TButton",
    ...
    def _iniciar_cadastro(self):
        """Ativa modo cadastro e mostra instruções; a captura é por clique."""
        ...
        if getattr(self, 'pose_detector', None):
            ...
            # alerta se já existia alguém com esse nome
            existing = self.pose_detector._duplicate_name_existing_ids
            extra = ""
            if existing:
                extra = f"\n⚠️ Já existem cadastros com este nome (IDs: {', '.join(existing)}). " \
                        f"Um novo ID será criado para este cadastro."

            self.info_text.set(
                f"Modo cadastro ATIVO para {nome}.{extra}\n\n"
                f"➡️ Serão capturadas {self.n_shots_default} fotos.\n"
                "A cada foto, SIGA a posição exibida na tela e clique em CAPTURAR.\n"
                "Posições incluem: frontal, olhar p/ esquerda/direita, cima/baixo e inclinações."
            )
    ...
    def _capturar_cadastro(self):
        """Captura UMA foto da sessão. Ao completar, finaliza e dá feedback."""
        ...
        if not result.get("ok"):
            self.info_text.set(f"⚠️ {result.get('msg')}")
            return
        ...
        if result.get("done"):
            ...
            if final.get("ok"):
                ...
                if final.get("duplicate"):
                    ...
                    if ids:
                        extra = f"\n⚠️ Nome duplicado: já existiam IDs {', '.join(ids)}. " \
                ...
                self.info_text.set(
                    ...
                    f"Contact sheet: {os.path.basename(final.get('page_jpg') or '—')} / "
                    f"{os.path.basename(final.get('page_pdf') or '—')}"
                    f"{extra}"
                )
                # overlay no vídeo por alguns segundos
                if hasattr(self.pose_detector, "flash_message"):
                    self.pose_detector.flash_message(f"Cadastrado: {name} [{pid}] ✔", seconds=5.0)

                self.status_text.set("Cadastro concluído")
                self.status_pill.configure(bg=self.COLORS["panel"], fg=self.COLORS["success"])
            else:
                self.info_text.set(f"❌ Falha ao finalizar cadastro: {final.get('msg')}")
                self.status_text.set("Cadastro falhou")
                self.status_pill.configure(bg=self.COLORS["panel"], fg=self.COLORS["danger"])
            self.cadastro_ativo = False
        else:
            self.info_text.set(
                f"📸 Foto {step}/{total} capturada. Faltam {remaining}.\n"
                f"Próxima posição: {next_prompt}\n"
    ...
    def _pose_loop_step(self):
        ...
        try:
            try:
                ...
                mensagens, annotated_frame, keypoints = [f"Erro na detecção: {e}"], None, None

            if mensagens:
                # Não sobrescreve feedbacks importantes do cadastro,
                # apenas acrescenta se houver algo novo.
                base = self.info_text.get()
                add = "\n".join(mensagens)
                if add and add not in base:
                    self.info_text.set(add if base.startswith("Informações") else (base + "\n" + add))
